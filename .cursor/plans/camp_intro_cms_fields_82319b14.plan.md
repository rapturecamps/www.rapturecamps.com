...
## Both EN and DE Templates

Both `src/pages/surfcamp/[country]/[camp]/index.astro` and `src/pages/de/surfcamp/[country]/[camp]/index.astro` already have the same intro section layout (just different hardcoded languages). Both will be updated identically to pull from CMS data.

Since camp documents are translatable via the document internationalization plugin, the EN camp document holds English values and the DE camp document holds German values. The templates already fetch with the correct `lang` parameter, so both languages are handled automatically from one set of schema fields.

The sidebar labels ("Location", "Surf Levels", etc. vs "Standort", "Surf-Level", etc.) stay hardcoded per template since they're UI chrome, not content.
