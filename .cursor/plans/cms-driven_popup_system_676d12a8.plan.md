...
todos:
  ...
  - id: update-popup-manager
    ...
    status: completed
  - id: zoho-integration
    content: Create /api/popup-submit endpoint for Zoho Campaigns email subscription, add zohoListKey field to schema
    status: completed
  - id: cleanup
...
- **Targeting rules**:
  - Target mode: `all-pages`, `url-patterns`, `url-contains`, or `specific-camps`
  - URL patterns: array of glob-like strings (e.g., `/surfcamp/bali/`*, `/de/surfcamp/bali/*`)
  - URL contains: array of keywords — popup shows if the URL contains any of them (e.g., `bali` matches camp pages AND blog posts mentioning Bali)
...
## 6. Zoho Campaigns Integration

Create [src/pages/api/popup-submit.ts](src/pages/api/popup-submit.ts) as a server-side API endpoint:

- Accepts POST with `{ email, listKey, source }` from the popup frontend
- Calls Zoho Campaigns Subscribe API: `POST https://campaigns.zoho.com/api/v1.1/json/listsubscribe`
- Uses OAuth token from env var `ZOHO_CAMPAIGNS_TOKEN`
- Passes the list key (configured per popup in Sanity) and source label
- Returns success/error to the client

Add fields to the popup schema:

- **Zoho List Key** (string) — the mailing list to subscribe to, configured per popup
- **Source Label** (string) — e.g., "Bali Spinning Wheel" for tracking in Zoho

Update the spinning wheel and any email-capturing popup to POST to `/api/popup-submit` instead of just logging to console.

Env vars needed: `ZOHO_CAMPAIGNS_TOKEN` (OAuth token for Zoho API auth).
...
- Create a "Bali Special Offer" popup in Sanity, set target to "URL contains" with keyword `bali`, and it shows on all Bali camp pages AND Bali-related blog posts
...
- Captured emails go directly to Zoho Campaigns mailing lists, with each popup feeding its own list
