...
overview: Wire up OG images with a fallback chain -- custom Sanity image (future) > first hero image > default branded fallback. For now, every page uses its hero image until Sanity is integrated.
todos:
  - id: baselayout-meta
