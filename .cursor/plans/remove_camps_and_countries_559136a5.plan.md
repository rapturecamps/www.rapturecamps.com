...
todos:
  ...
  - id: remove-astro-redirect
    ...
    status: completed
  - id: make-country-page-sanity-only
    content: Make country page work without hardcoded fallback so new countries (e.g. Sri Lanka) can be added purely from Sanity
    status: completed
  - id: verify-build
...
#### 9. Make country page Sanity-only capable — [src/pages/surfcamp/[country].astro](src/pages/surfcamp/[country].astro)

- Remove the `if (!fallback) return Astro.redirect("/404")` guard (line 388)
- Make the `data` object build from Sanity first, falling back to `countryContent` only if Sanity has no data
- If neither Sanity nor fallback has data, then 404
- This allows adding new countries like Sri Lanka purely from Sanity Studio without code changes
...
#### 11. Migration scripts (optional cleanup)
