...
todos:
  - id: hero-h1
    content: Fix H1 on homepage/country pages -- add headingTag prop to Hero.astro
    status: completed
  - id: multiple-h1
...
### 1. No H1 on homepage and country pages

[src/components/sections/Hero.astro](src/components/sections/Hero.astro) line 104 renders the main heading as `<p role="heading" aria-level="2">` instead of a semantic `<h1>`. Homepage, country pages, and contact page all rely on Hero for their primary heading.

**Fix:** Add a prop (e.g. `headingTag="h1"`) to Hero. When set, render an `<h1>` instead of `<p>`. Pass it from homepage, country, and contact pages.
